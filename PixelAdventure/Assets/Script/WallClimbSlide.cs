...
{
    private bool isTouchingWall; // Trạng thái đang tiếp xúc với tường
    ...
    // Xác định xem nhân vật đã chạm tường nhưng chưa chạm đất hoặc chưa chạm vào bất kỳ đối tượng nào
    ...
    private void Update()
    {
        ...
        if (!isTouchGrounded)
        {
            ...
            if (Physics2D.Raycast(transform.position, transform.right, wallJumpCheckDistance, wallLayer))
            {
                isTouchingWall = true;

            }
            else if (Physics2D.Raycast(transform.position, -transform.right, wallJumpCheckDistance, wallLayer))
            {
                isTouchingWall = true;
            }
            else
